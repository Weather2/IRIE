...
c
	close(14)
	open(14,file=gdsfile,status='old',form='unformatted')
       read(14) GDS
...
c===============================================================================
c
      subroutine getdata(fname,pr,uw,vw,ht,mr,cr,sfcgrid)
c
...
c
      integer*4 nx,ny,nz,dummy(7),nsfcfld,i,j,k,pres
c
...
c
     ...
     .      ,cr(nx,ny,nz)    !cloud mixing ratio (kg/kg)
     .      ,pr(nz)          !pressure (Pa)
c
      character*(*) fname
      character*2   gproj
...
c
	write(6,*) 'reading in getdata ', nx,ny,nz
      ...
      read(1) (((cr(i,j,k),i=1,nx),j=1,ny),k=nz,1,-1)

	write(6,*) 'U,V,Z,Q,C : ', uw(nx/2,ny/2,5),vw(nx/2,ny/2,5),
     +			ht(nx/2,ny/2,5),mr(nx/2,ny/2,5),cr(nx/2,ny/2,5)
