      SUBROUTINE READSFC(HLAT,HLON,SM,SST,SICE,SMC,STC,TG, SNO)
C     ******************************************************************
C
C	Needed inputs:
C
...
C----------------------------------------------------------------------
       ...
       REAL HLAT(IM,JM),HLON(IM,JM), SNO(IM,JM), SST(IM,JM),TG(IM,JM)
                             D I M E N S I O N
     &  IGDATE(4)
C
...
C	btime=etime()
cpeja 
      open(52,file="../init_out/sfc.dat",form='unformatted'
     +              ,status='old')
      print *,IM,JM,' Nsoil=',NSOIL
      read (52)  SICE
      read (52)  SNO
      read (52)  SST 
      print *,' read ok!'
