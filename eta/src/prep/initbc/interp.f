...
c
	...
	ALLOCATE(SFCGRID(NXIN,NYIN,12))
      call getdata(init_in(nnn),prin,uwin,vwin,htin,mrin,sfcgrid)
...
c
	...
	if (SIGMA) then
      ...
      call vinterp(prin,hthi,mrhi,uwhi,vwhi,sfcgridhi,nzin,idat)
	endif
c
      return
      end
...
Cold	DO N=1,2
	...
	DEALLOCATE(VTMP)
	 ...
	 call ltlwin(ald,apd,utmpout,vtmpout,dtr,utmpout,vtmpout)

	do j=1,jm
        do I=1,im
	  uint(i,j,l)=utmpout(i,j)
	  vint(i,j,l)=vtmpout(i,j)
        enddo
        enddo
...
c==========================================================================
C
      subroutine vinterp(spl,h,q,u,v,sfcgrid,ldm,idat)
c
