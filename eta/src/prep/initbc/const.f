...
c
     ...
     .      ,pd(im,jm)
     .      ,pdt(im,jm),fist(im,jm),ivhgt(im,jm)  
...
C	       L   I3    I    
	...
	write(6,*) 'middle of preproc.init read'
      read(1) tt,qt,ct,pdt,fist,sm,res,eta,pt,detac,aeta,dfl            
...
C------------------------------
      do l=1,lm
        ...
        enddo
         write(1) dum2
        if (l.eq.20) print *,'cwm:',dum2(1,1),dum2(21,21),dum2(im,jm)
      enddo
        DEALLOCATE(CT)
C------------------------------
C	wet --> si
...
C-------------------------------------
      ...
      ivhgt=fist/9.81
      open(1,file='../input_data/hgt_sm.dat'
     .    ,status='unknown',form='unformatted')
      write(1) ivhgt,sm
      close(1)
...
C Constants/data used in cold start SH2O initialization
      REAL HLICE, GRAV, BLIM, T0K
      DATA HLICE/3.335E5/,GRAV/9.81/,T0K/273.15/
      DATA BLIM/5.5/
      DATA PSIS   /0.069, 0.036, 0.141, 0.759, 0.759,
     &             0.355, 0.135, 0.617, 0.263, 0.098,
     &             0.324, 0.468, 0.355, 0.000, 0.069,
     &             0.036, 0.468, 0.069, 0.069/
      DATA BETA   /2.79,  4.26,  4.74,  5.33,  5.33,
     &             5.25,  6.66,  8.72,  8.17, 10.73,
     &            10.39, 11.55,  5.25,  0.00,  2.79,
     &             4.26, 11.55,  2.79,  2.79/
      DATA SMCMAX /0.395, 0.421, 0.434, 0.476, 0.476,
     &             0.439, 0.404, 0.464, 0.465, 0.406,
     &             0.468, 0.468, 0.439, 1.000, 0.200,
     &             0.421, 0.468, 0.200, 0.395/
      REAL DELTA
      INTEGER ITYP
      DATA DRYSMC /0.011, 0.028, 0.047, 0.084, 0.084,
     &             0.066, 0.067, 0.120, 0.103, 0.100,
     &             0.126, 0.138, 0.066, 0.000, 0.006,
     &             0.028, 0.138, 0.006, 0.011/
C-----------------------------------------------------------------------
...
C	removed squaring per t. black recommendation
	...
	coac=.04*(.53846154/DPHD)*0.5

C	cap at 0.2
cir	coac=amin1(coac,0.2)
       ...
       DO J=1,JM
        DO I=1,IM-1+MOD(J,2)
           IF(ETA(L+1).GT.RES(I,J)) THEN
                  ...
                  JHS=J-1
                  JHN=J+1
                  IF(IHE.GE.1.AND.IHE.LE.IM)VTM(IHE,J,L)=0.
                  IF(IHW.GE.1.AND.IHW.LE.IM)VTM(IHW,J,L)=0.
                  IF(JHS.GE.1.AND.JHS.LE.JM)VTM(I,JHS,L)=0.
                  IF(JHN.GE.1.AND.JHN.LE.JM)VTM(I,JHN,L)=0.
           ENDIF
        ENDDO
       ENDDO
      ENDDO
...
CIVAN_NEW
!IV         OPEN (UNIT=31,file='../input_data/albedo.alb'
!IV     &        ,STATUS='OLD',FORM='UNFORMATTED')
!IV         READ (31) ALBASE
!IV         CLOSE (UNIT=31)
!IV         print *,'READ NEW ALBEDO '
CIVAN_NEW
...
Cmp    END ALBEDO SECTION **************************************************
	...
	write(6,*) 'back from SSTHIRES call'
	endif
...
Cmp	operational line only used if dont have snowdepth data
	...
	IF (.NOT. GRIBSOIL) THEN
	  CALL READSFC(glat,glon,sm,SST,sice,smc,stc,tg,sno)
          DO J=1,JM
          DO I=1,IM
...
CIVAN LAND
          ...
          END DO
        ELSE
          CALL READSFC(glat,glon,sm,dum2,sice,smc,stc,tg,dum2)
	END IF
...
C      SET DEFAULT FOR ETA LAND FAR FROM LAND IN GLOBAL
       IONETA=7
       CALL PUTEM(glat,glon,I1D,sm,sice,IONETA)
CIVAN_NEW
       OPEN (UNIT=31,file='../input_data/soil.veg'
     &,      STATUS='OLD',FORM='UNFORMATTED')
       READ (31)IONETA
       CLOSE (UNIT=31)
       print *,'READ REAL VEGETA'
CIVAN_NEW
	...
	write(6,*) 'sample veg type values'
        do J=jm,1,-jm/32
	write(6,429) (IONETA(I,J),I=1,im,im/16)
  429	format(41(I3,x))
	enddo
...
C      SET DEFAULT FOR ETA LAND FAR FROM LAND IN GLOBAL
       ...
       END DO
CIVAN_NEW
       OPEN (UNIT=31,file='../input_data/soil.soil'
     &,      STATUS='OLD',FORM='UNFORMATTED')
       READ (31)IONETA
       print *,'READ REAL SOIL'
       CLOSE (UNIT=31)
CIVAN_NEW
       ...
       WRITE(1) IONETA
        ...
        do J=jm,1,-jm/32
        write(6,439) (IONETA(I,J),I=1,im,im/16)
  439   format(41(I3,x))
        enddo
...
C Add sh2o stuff right here
	...
	DO 70 K=1,NSOIL
CIVAN-SOIL
       IF(SM(I,J).eq.0.AND.SICE(I,J).EQ.0.)THEN
         IF(SMC(I,J,K).GT.0.31)THEN
           SMC(I,J,K)=MIN(0.714*SMC(I,J,K)+0.089,0.36)
         ENDIF
         IF(SMC(I,J,K).LT.DRYSMC(IONETA(I,J)))
     &   SMC(I,J,K)=DRYSMC(IONETA(I,J))
         IF(SMC(I,J,K).GT.SMCMAX(IONETA(I,J)))
     &   SMC(I,J,K)=SMCMAX(IONETA(I,J))
       ENDIF
C ----------------------------------------------------------------------
...
Cnew
C
            IF (STC(I,J,K).LT.273.149.AND.SICE(I,J).EQ.0.) THEN
C ----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
	...
	CALL VFRAC(im,jm,glatr,glonr,TLM0D,TPH0D,DLMD,DPHD,sm,sice,
     +	vegfrc)
   
CIVAN_NEW
!IV         OPEN (UNIT=31,file='../input_data/gfrac.gfrac'
!IV     &        ,STATUS='OLD',FORM='UNFORMATTED')
!IV         READ (31) VEGFRC
!IV         CLOSE (UNIT=31)
!IV         print *,'READ REAL VEGFRC '
CIVAN_NEW
