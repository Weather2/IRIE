        subroutine pusi(spl,htll,qtll,ctll,utll,vtll,sfcgrid,ldm,idat)
!     *************************************************************     
!     *                                                           *
...
!-----------------------------------------------------------------------
     ...
     &,t(im,jm,lm),q(im,jm,lm),c(im,jm,lm)
     &,z(im,jm,lm+1),alpi(im,jm,lm+1)
                             d i m e n s i o n
     & pdb(kb,2)
     &,ub(kb,lm,2),vb(kb,lm,2),tb(kb,lm,2),qb(kb,lm,2),cb(kb,lm,2)
...
!
      d1=(cld0-cld1)/(zld0-zld1)
!      d2=(d1-(cld1-cld2)/(zld1-zld2))/(zld0-zld2)
      d2=0.
!
      cij(lold)=d2*x*x+d1*x+cld0
...
!
      ...
      call spline_pus(lold,zslh,cij,y2,lm,zqtil,ctil,pp,qq)
!
          do l=1,lm
      qp=amax1(qtil(l),epsq)
      t(i,j,l)=t(i,j,l)/(qp*0.608+1.)
      q(i,j,l)=qp
      c(i,j,l)=amax1(ctil(l),0.)
...
Ctst
      ...
      write(NFCSTE) t,q,c,pd,fis,sm,ref,sg,pt,dsg,sgml,dfl
...
!      print*,ub
     ...
     .     ,status='unknown',form='unformatted')
      write(1) run,idat(1),idat(2),idat(3),idat(4),pd,t,q,u,v,c
