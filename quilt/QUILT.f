...
C
      ...
      ALLOCATE(CFRACALL(MY_ISD:MY_IED,MY_JSD:MY_JED,1:LM))
      ALLOCATE(CFRACCON(MY_ISD:MY_IED,MY_JSD:MY_JED,1:LM))
      ...
      ALLOCATE(CFRACO(MY_ISD:MY_IED,MY_JSD:MY_JED))
...
c  50   FORMAT('restrt',I3.3,'.',I3.3)
   ...
   50   FORMAT('restrt',I3.3,'.quilt')
      ELSE
       WRITE(RSTFIL1,55)IHOUR,IPE,RESTHR
  55   FORMAT('restrt',I3.3,'.',I3.3,'.quilt.',a4)
!dule        WRITE(RSTFIL1,55)IHOUR,RESTHR
!dule   55   FORMAT('restrt',I3.3,'.quilt.',a4)
      ENDIF
C
...
C-----------------------------------------------------------------------
!dule      DO 200 IPE=JSTA(ME), JEND(ME)
C-----------------------------------------------------------------------
C***
...
!dule      read(LRSTRT1,rec=ipe+1,iostat=ier) (buf(i),i=1,irecl/4)
      read(LRSTRT1,rec=1,iostat=ier) (buf(i),i=1,irecl/4)
      if ( ier .ne. 0 ) then
...
c       READ(LRSTRT1)((CFRACALL(I,J,L),I=MY_IS_GLB,MY_IE_GLB),
c    1                              J=MY_JS_GLB,MY_JE_GLB)
      call decoal(cfracall(is:ie,js:je,l),len_ch)
c       READ(LRSTRT1)((CFRACCON(I,J,L),I=MY_IS_GLB,MY_IE_GLB),
c    1                              J=MY_JS_GLB,MY_JE_GLB)
...
cwas    WRITE(LRSTRT2)((TCUCN(I,J,L),I=1,IM),J=1,JM)
        ...
        IF ( LME ) WRITE(LRSTRT2) DUM1
cwas    WRITE(LRSTRT2)((CFRACALL(I,J,L),I=1,IM),J=1,JM)
        CALL COLLECT(CFRACALL(:,:,L),DUM1)
        IF ( LME ) WRITE(LRSTRT2) DUM1
cwas    WRITE(LRSTRT2)((CFRACCON(I,J,L),I=1,IM),J=1,JM)
