...
C     
      ...
      REAL CLOUDALL(IM,JM),CLOUDCON(IM,JM)
...
CMEB 12/22/98 SWITCH TO RH VS WATER NO MATTER WHAT
              ...
              BQ2  =Q2A
CIVAN
              BIV  =CFRACALL(I,J,LMH(I,J))
              BIV1 =CFRACCON(I,J,LMH(I,J))
CIVAN
              AHF  =D00
              AHFQ =D00
              AHFO =D00
...
CIVAN
              BIV   =CFRACALL(I,J,NL1X(I,J))
              BIV1  =CFRACCON(I,J,NL1X(I,J))
CIVAN
C
...
C
     ...
     1              (ALPINT(I,J,NL1X(I,J)+1)-ALPINT(I,J,NL1X(I,J)-1))
CIVAN
              AHFIV =(BIV-CFRACALL(I,J,NL1X(I,J)-1))/
     1              (ALPINT(I,J,NL1X(I,J)+1)-ALPINT(I,J,NL1X(I,J)-1))
              AHFIV1=(BIV1-CFRACCON(I,J,NL1X(I,J)-1))/
     1              (ALPINT(I,J,NL1X(I,J)+1)-ALPINT(I,J,NL1X(I,J)-1))
CIVAN
            ENDIF
C
...
CIVAN
            CLOUDALL(I,J)=BIV+AHFIV*(TRF-FAC)
            CLOUDALL(I,J)=AMAX1(CLOUDALL(I,J),0.)
            CLOUDCON(I,J)=BIV1+AHFIV1*(TRF-FAC)
            CLOUDCON(I,J)=AMAX1(CLOUDCON(I,J),0.)
CIVAN
...
cpeja 
c       print *,i,j,fis(i,j),PSLP(i,j),FSL(i,J),ALPSL, Z1000(i,j)
              ENDDO
              ENDDO
c       stop
...
C           
         IF (IGET(184).GT.0) THEN
          IF (LVLS(L,IGET(184)).GT.0) THEN
            CALL E2OUT(184,000,CLOUDALL,EGRID2,GRID1,GRID2,IMOUT,JMOUT)
            CALL BOUND(GRID1,D00,H99999,IMOUT,JMOUT)
            ID(1:25)=0
            CALL OUTPUT(IOUTYP,IGET(184),L,GRID1,IMOUT,JMOUT)
          ENDIF
         ENDIF
C
C***  IVAN CONVECTIVE CLOUD
C
         IF (IGET(185).GT.0) THEN
          IF (LVLS(L,IGET(185)).GT.0) THEN
            CALL E2OUT(185,000,CLOUDCON,EGRID2,GRID1,GRID2,IMOUT,JMOUT)
            CALL BOUND(GRID1,D00,H99999,IMOUT,JMOUT)
            ID(1:25)=0
            CALL OUTPUT(IOUTYP,IGET(185),L,GRID1,IMOUT,JMOUT)
          ENDIF
         ENDIF
C     
