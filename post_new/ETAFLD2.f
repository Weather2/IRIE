...
C     FIRST, MESINGER'S/Zavisa's SEA LEVEL PRESSURE.
      IF (IGET(023).GT.0) THEN
         CALL E2OUT(023,000,PSLP,EGRID2,GRID1,GRID2,IMOUT,JMOUT)
         ID(1:25) = 0
         print*,'printing output mesinger pslp'
         do i=1,im
         do j=jsta,jend
C         print*,i,j,grid1(i,j)
         end do
         end do
c         print*,'OUTPUT MESINGER SEA LEVEL PRESSURE'
...
C     SECOND, STANDARD NGM SEA LEVEL PRESSURE.
      IF (IGET(105).GT.0) THEN
         ...
         ID(1:25) = 0
         print*,'printing output NGM pslp'
         do i=1,im
         do j=jsta,jend
C         print*,i,j,grid1(i,j)
         end do
         end do
         print*,'OUTPUT NGM SEA LEVEL PRESSURE'
...
C           PRESSURE ON ETA SURFACES.
            IF (IGET(001).GT.0) THEN
             IF (LVLS(L,IGET(001)).GT.0) THEN
               ...
               print*,'OUTPUT PRESSURE ON ETA SURFACE'
...
C           HEIGHTS ON ETA SURFACES.
            IF (IGET(077).GT.0) THEN
             IF (LVLS(L,IGET(077)).GT.0) THEN
               ...
               LYR = L
               IF (ITYPE.GT.1) ID(9) = 109
               print*,'OUTPUT HEIGHT on ETA SURFACE'
