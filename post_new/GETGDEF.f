...
c
	...
	IMJM=IM*JM-JM/2
        ALLOCATE(KHL0(1:JM))
        ALLOCATE(KHH0(1:JM))
        ALLOCATE(GLAT(1:IMJM))
        ALLOCATE(GLON(1:IMJM))
C
...
Cmp end dateline add
        ...
        write(6,*) 'jdim ',jdim,nlat,slat,dlat
Cmp        idim=((elon-wlon)/dlat)+1
        idim=((eloncalc-wloncalc)/dlat)+1
        write(6,*) 'idim ',idim,eloncalc,wloncalc,dlat
        islat=slat*1000
        iwlon=wlon*1000
        nlat=nlat*1000
Cmp
...
C	tlmuse=TLM0D
  ...
  900	print*, 'end of namelist file'
  901	print*, 'model list trouble'
        DEALLOCATE(KHL0)
        DEALLOCATE(KHH0)
        DEALLOCATE(GLAT)
        DEALLOCATE(GLON)
